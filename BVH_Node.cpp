...
extern vector<VertexStatus> verticesStatus;
...
void changeState(BVH_Node *temp, VertexStatus status) {
    if (status == NORMAL) { // change to normal special consideration
        ...
        if (verticesStatus[temp->node[1]] == CLOTH_COLLISION)
            verticesStatus[temp->node[1]] = NORMAL;
...
bool BVH_Node::leafCollide(BVH_Node *target) {
    ...
    if (target->isLeaf()) {
        ...
        if (ret == true) {
            changeState(this, CLOTH_COLLISION);
