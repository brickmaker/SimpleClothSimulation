...
//    velocities[temp->node[0]].y = 0;
//    velocities[temp->node[1]].y = 0;
//    velocities[temp->node[2]].y = 0;
...
BVH_Node::BVH_Node(int startX, int startY, int endX, int endY) : isleaf(false) {
    ...
    if (endX - startX == 1 && endY - startY == 1) {
        // 这里没有用到那个诡异的三角形分割
        _leftChild = new BVH_Node(XYtoIndex(startX, startY), XYtoIndex(startX + 1, startY),
                                  XYtoIndex(startX, startY + 1));
...
void intersect(BVH_Node *target1, BVH_Node *target2) {
//    stopMove(target1);
//    stopMove(target2);
    glm::vec3 average1(0), average2(0);
    average1 += velocities[target1->node[0]];
    average1 += velocities[target1->node[1]];
    average1 += velocities[target1->node[2]];

    average2 += velocities[target2->node[0]];
    average2 += velocities[target2->node[1]];
    average2 += velocities[target2->node[2]];
////    glm::vec3 *from = glm::length(average1) > glm::length(average2) ? &average1 : &average2;
////    glm::vec3 *to = glm::length(average1) <= glm::length(average2) ? &average1 : &average2;
//
//
    average1 /= 3;
    average2 /= 3;

    velocities[target1->node[0]] = average2;
    velocities[target1->node[1]] = average2;
    velocities[target1->node[2]] = average2;
    velocities[target2->node[0]] = average1;
    velocities[target2->node[1]] = average1;
    velocities[target2->node[2]] = average1;
//    //target1->node[0]
//
//    average = glm::vec3(0);
//    average += forces[target1->node[0]];
//    average += forces[target1->node[1]];
//    average += forces[target1->node[2]];
//    average += forces[target2->node[0]];
//    average += forces[target2->node[1]];
//    average += forces[target2->node[2]];
//    average /= 6;
//
//    forces[target1->node[0]] = average;
//    forces[target1->node[1]] = average;
//    forces[target1->node[2]] = average;
//    forces[target2->node[0]] = average;
//    forces[target2->node[1]] = average;
//    forces[target2->node[2]] = average;
