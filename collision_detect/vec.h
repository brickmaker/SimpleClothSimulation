...
template<unsigned int N, class T>
T mag(const Vec<N, T> &a) { return (T) sqrt(mag2(a)); }
...
inline Vec<3, T> rotate(const Vec<3, T> &x, const T &angle, const Vec<3, T> &u) {
    ...
    return Vec<3, T>(
            u[0] * (ux + vy + wz) + (x[0] * (u[1] * u[1] + u[2] * u[2]) - u[0] * (vy + wz)) * ca + (-wy + vz) * sa,
...
template<class T>
inline void minmax(const T &x0, const T &x1, T &xmin, T &xmax);

template<class T>
inline void minmax(const T &x0, const T &x1, const T &x2, T &xmin, T &xmax);

template<class T>
inline void minmax(const T &x0, const T &x1, const T &x2, const T &x3,
                   T &xmin, T &xmax);

template<class T>
inline void minmax(const T &x0, const T &x1, const T &x2, const T &x3, const T &x4,
                   T &xmin, T &xmax);

template<class T>
inline void minmax(const T &x0, const T &x1, const T &x2, const T &x3, const T &x4,
                   const T &x5, T &xmin, T &xmax);

template<unsigned int N, class T>
inline void update_minmax(const Vec<N, T> &x, Vec<N, T> &xmin, Vec<N, T> &xmax);

template<unsigned int N, class T>
void minmax(const Vec<N, T> &x0, const Vec<N, T> &x1, Vec<N, T> &xmin, Vec<N, T> &xmax) {
    for (unsigned int i = 0; i < N; ++i)
        minmax(x0.v[i], x1.v[i], xmin.v[i], xmax.v[i]);
}

template<unsigned int N, class T>
void minmax(const Vec<N, T> &x0, const Vec<N, T> &x1, const Vec<N, T> &x2, Vec<N, T> &xmin, Vec<N, T> &xmax) {
    for (unsigned int i = 0; i < N; ++i)
        minmax(x0.v[i], x1.v[i], x2.v[i], xmin.v[i], xmax.v[i]);
}

template<unsigned int N, class T>
void minmax(const Vec<N, T> &x0, const Vec<N, T> &x1, const Vec<N, T> &x2, const Vec<N, T> &x3, Vec<N, T> &xmin,
            Vec<N, T> &xmax) {
    for (unsigned int i = 0; i < N; ++i)
...
template<unsigned int N, class T>
void minmax(const Vec<N, T> &x0, const Vec<N, T> &x1, const Vec<N, T> &x2, const Vec<N, T> &x3, const Vec<N, T> &x4,
            Vec<N, T> &xmin, Vec<N, T> &xmax) {
    for (unsigned int i = 0; i < N; ++i)
...
template<class T>
void minmax(const T &x0, const T &x1, T &xmin, T &xmax) {
    std::initializer_list<T> s = {x0, x1};
    xmax = std::max(s);
    xmin = std::min(s);
}

template<class T>
void minmax(const T &x0, const T &x1, const T &x2, T &xmin, T &xmax) {
    std::initializer_list<T> s = {x0, x1, x2};
    xmax = std::max(s);
    xmin = std::min(s);
}

template<class T>
void minmax(const T &x0, const T &x1, const T &x2, const T &x3, T &xmin, T &xmax) {
    std::initializer_list<T> s = {x0, x1, x2, x3};
    xmax = std::max(s);
    xmin = std::min(s);
}

template<class T>
void minmax(const T &x0, const T &x1, const T &x2, const T &x3, const T &x4, T &xmin, T &xmax) {
    std::initializer_list<T> s = {x0, x1, x2, x3, x4};
    xmax = std::max(s);
    xmin = std::min(s);
}

template<class T>
void minmax(const T &x0, const T &x1, const T &x2, const T &x3, const T &x4, const T &x5, T &xmin, T &xmax) {
    std::initializer_list<T> s = {x0, x1, x2, x3, x4, x5};
    xmax = std::max(s);
    xmin = std::min(s);
}

template<unsigned int N, class T>
void update_minmax(const Vec<N, T> &x, Vec<N, T> &xmin, Vec<N, T> &xmax) {
